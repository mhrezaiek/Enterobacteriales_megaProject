...
def Label_creator(raw_table_path,Isolates_table_path):
    ...
    drugs = ["piperacillin","cefepime","aztreonam","tobramycin","ciprofloxacin","gentamicin",
             "imipenem", "ceftazidime","colistin","meropenem","amikacin","levofloxacin"]

    for i in range(len(isolates_table)-1):
        phenotype = ""
        measurement_value = ""
        gene_id = isolates_table[i + 1][1]
            ...
            if gene_id == test_gene:
                for drug in range(len(drugs)):
                    if antibiotic == drugs[drug]:
                        ...
                        raw_table[gene][drug+1+12] = measurement_value
    dataset = pd.DataFrame(raw_table)
    dataset.to_csv("test1.csv", index=False)


if __name__ == "__main__":
    Label_creator("../Peseudomonas/datasets/raw_dataset.csv", "../Peseudomonas/datasets/PATRIC_genome_amr.csv")
