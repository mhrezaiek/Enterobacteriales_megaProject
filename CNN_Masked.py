...
def model(input_shape, Nlayer):
    ...
    #x =Conv1D(10,kernel_size= 10, padding = 'same', kernel_initializer=RandomUniform(), bias_initializer=TruncatedNormal(), activation='relu')(input1)
    x = Dense(10, kernel_initializer=RandomUniform(), bias_initializer=TruncatedNormal(), activation='relu' )(input1)
    x = tf.keras.layers.Reshape([10, 1])(x)
    for i in range(Nlayer):
        n = int(n / 2)
        x =Conv1D(n,kernel_size= 10, padding = 'same', kernel_initializer=RandomUniform(), bias_initializer=TruncatedNormal(), activation='relu')(x)
...
#data = genome_id_creator("../reduced_snp_dataset_17drugs.csv")
...
#dataset = np.asarray(dataset)
##dataset = np.reshape(dataset, (-1,378967,1))
#y = np.asmatrix(data.iloc[0:,378971:])
...
#for i in range(0,17):
    #for j in range(y_score.shape[0]):
       # if y_test[j,i] != -1:
            ...
            #y_score = np.delete(y_score, j, axis=0)
